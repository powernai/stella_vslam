...
class Processor:
    ...
    def localize(self):
        ...
        '''
        subprocess.run([
            './run_video_slam', '-v', './orb_vocab.fbow', '-m', self.video_local_path, '-c', self.camera_local_path, '--frame-skip', str(
                self.skip_frame), '--no-sleep',
    ...
    def extract_frames_and_coordinates(self):
        ...
        '''
        service = FrameCoordinatesExtractor(
            self.output_path, self.video_local_path, self.dest_path)
    ...
    def upload(self):
        ...
        try:
            self._S3_READ_CLIENT.head_object(
                Bucket=self.bucket, Key=self.parent_key + f"/{self.folder_path}")
            print("Folder already exists in S3, key: ",
                  self.parent_key + f"/{self.folder_path}")
        except botocore.exceptions.ClientError as e:
            if e.response['Error']['Code'] == '404':
                self._S3_READ_CLIENT.put_object(
                    Bucket=self.bucket, Key=self.parent_key + f"/{self.folder_path}")
                self._S3_READ_CLIENT.put_object(
                    Bucket=self.bucket, Key=self.parent_key + f"/{self.coordinates_path}")
                print("Created folder in S3, key: ",
                      self.parent_key + f"/{self.folder_path}")

        # get all files to upload
        files = os.listdir(self.dest_path)
        files_to_upload = [os.path.join(self.dest_path, file)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            batches = [files_to_upload[i:i+batch_size]
            ...
            for batch in batches:
                ...
                futures.extend([executor.submit(self.upload_batch, self.bucket, file_name, self.parent_key +
                               f"/{self.folder_path}{file_name.split('/')[-1]}") for file_name in batch])
        ...
        with open(file_obj_path, 'rb') as fp:
            self._S3_READ_CLIENT.upload_fileobj(
                Fileobj=fp, Bucket=self.bucket, Key=self.parent_key + f"/{self.coordinates_path}{self.coordinates_file}")
        print(f"Uploaded coordinates.json file ")

        self.images_url = Utils.get_object_url(
            self.bucket, self.parent_key + f"/{self.folder_path}")
        self.coordinates_url = Utils.get_object_url(
            self.bucket, self.parent_key + f"/{self.coordinates_path}{self.coordinates_file}")
    ...
    def cleanup(self):
        ...
        file_paths = [self.video_local_path, self.camera_local_path,
                      self.output_path, self.coordinates_file, self.dest_path]
...
if __name__ == '__main__':
    ...
    all_videos_urls = ast.literal_eval(
        os.environ.get('ALL_VIDEO_PATH', str([])))
    all_camera_urls = ast.literal_eval(
        os.environ.get('ALL_CAMERA_PATH', str([])))
    ...
    for i in range(total_count):
        ...
        print(f"Processing {i+1} of {total_count}",
              all_videos_urls[i].get('url'))
        ...
        environ['DATE'] = datetime.datetime.strptime(
            all_dates[i], "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%Y-%m-%d")
        environ['processing_count'] = str(i+1)

        processor = Processor(video_path, camera_path,
                              video_version_id, camera_version_id)
        processor.process()
